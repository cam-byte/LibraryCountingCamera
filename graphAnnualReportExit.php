...
<script type="text/javascript">
  ...
  <nav>
    ...
    <div class="dailyreportanalysis"  style = "position:relative; left:250px; top:-600px;">
      ...
      <?php $result = mysqli_query($conn,"SELECT MIN(NULLIF(PeopleExited,0)) AS min_people FROM AnnualReportDataExited");
            $row = mysqli_fetch_array($result);
            $minpeople = $row['min_people'];
            $result2 = mysqli_query($conn,"SELECT Month FROM AnnualReportDataExited WHERE PeopleExited=$minpeople");
            $row2 = mysqli_fetch_array($result2);
            $mintime = $row2['Month'];
      ?>
      <p>The least people entered(<?php echo $minpeople?>) was in <?php echo $mintime  ?>
    </div>
