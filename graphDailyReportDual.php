...
<script type="text/javascript">
  ...
  <body>
    ...
    <div class="small-chart-container" style = "position:relative; left:550px; top:0px;">
       <div id="line_top_x" style="position:relative; left:100px; top:100px;"></div>    
       <div id="chart_div" style="width: 400px; height: 120px; position:relative; left:-1300px; top:-400px;"></div>
    </div>
    <div class="dailyreportanalysis" style = "position:relative; left:250px; top:-600px;">
      ...
      <br>
      <?php $resultentered = mysqli_query($conn,"SELECT MIN(NULLIF(PeopleEntered,0)) AS min_people FROM DailyReportData");
            $row = mysqli_fetch_array($resultentered);
            $minpeopleenter = $row['min_people'];

            $resultentertime = mysqli_query($conn,"SELECT TimeEntered FROM DailyReportData WHERE PeopleEntered=$minpeopleenter");
            $row2 = mysqli_fetch_array($resultentertime);
            $mintimeenter = $row2['TimeEntered'];

            $resultexited = mysqli_query($conn,"SELECT MIN(NULLIF(PeopleExited,0)) AS min_peopleexit FROM DailyReportDataExited");
            $row3 = mysqli_fetch_array($resultexited);
            $minpeopleexited = $row3['min_peopleexit'];

            $resultexitedtime = mysqli_query($conn,"SELECT TimeExited FROM DailyReportDataExited WHERE PeopleExited=$minpeopleexited");
            $row4 = mysqli_fetch_array($resultexitedtime);
            $mintimeexited = $row4['TimeExited'];
      ?>
      <br>
