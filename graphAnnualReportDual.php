    ...
    <script type="text/javascript">
      ...
      function drawStuff() {
        var data = new google.visualization.arrayToDataTable([
          ...
          <?php 
          $sql = "SELECT * FROM AnnualReportData INNER JOIN AnnualReportDataExited ON AnnualReportData.Month=AnnualReportDataExited.Month";
...
<script type="text/javascript">
  ...
  <body>
    <div id="dual_x_div" style="width: 900px; height: 500px;position:relative; left:800; top:50px;"></div>
    <nav>
    ...
    <div class="dailyreportanalysis"   style = "position:relative; left:250px; top:-600px;">
      <p>The most people entered(<?php echo $max?>) was in <?php echo $maxtime  ?>
      <p>The most people exited(<?php echo $maxexit?>) was in <?php echo $maxtimeexit ?>
      <br>
      <?php $resultentered = mysqli_query($conn,"SELECT MIN(NULLIF(PeopleEntered,0)) AS min_people FROM AnnualReportData");
            $row = mysqli_fetch_array($resultentered);
            $minpeopleenter = $row['min_people'];

            $resultentertime = mysqli_query($conn,"SELECT Month FROM AnnualReportData WHERE PeopleEntered=$minpeopleenter");
            $row2 = mysqli_fetch_array($resultentertime);
            $mintimeenter = $row2['Month'];

            $resultexited = mysqli_query($conn,"SELECT MIN(NULLIF(PeopleExited,0)) AS min_peopleexit FROM AnnualReportDataExited");
            $row3 = mysqli_fetch_array($resultexited);
            $minpeopleexited = $row3['min_peopleexit'];

            $resultexitedtime = mysqli_query($conn,"SELECT Month FROM AnnualReportDataExited WHERE PeopleExited=$minpeopleexited");
            $row4 = mysqli_fetch_array($resultexitedtime);
            $mintimeexited = $row4['Month'];
      ?>
      <br>
      <p>The least people entered(<?php echo $minpeopleenter ?>) was in <?php echo $mintimeenter  ?>
      <p>The least people exited(<?php echo $minpeopleexited?>) was in <?php echo $mintimeexited?>
    </div>
