    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ...
          <?php
            $sql = "SELECT * FROM MonthlyReportDataExited";
...
<script type="text/javascript">
  ...
  <nav>
    ...
    <div id="line_top_x" style="width: 900px; height: 500px; position:relative; left:120px; top:90px;"></div>    
    ...
    <div class="dailyreportanalysis" style = "position:relative; left:250px; top:-600px;">
      ...
      <br>
      <?php $result = mysqli_query($conn,"SELECT MIN(NULLIF(PeopleExited,0)) AS min_people FROM MonthlyReportDataExited");
            $row = mysqli_fetch_array($result);
            $minpeople = $row['min_people'];
            $result2 = mysqli_query($conn,"SELECT DayOfMonth FROM MonthlyReportDataExited WHERE PeopleExited=$minpeople");
            $row2 = mysqli_fetch_array($result2);
            $mintime = $row2['DayOfMonth'];
      ?>
      <p>The least people entered(<?php echo $minpeople?>) was on day <?php echo $mintime  ?> of the month
    </div>
