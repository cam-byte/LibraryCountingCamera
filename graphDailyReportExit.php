...
$userselect= $_SESSION['login_user'];
          $minnumquery = "SELECT MIN(NULLIF(PeopleEntered,0)) FROM DailyReportData AS MIN_PRICE "; 
...
<script type="text/javascript">
  ...
  <nav>
		<ul id="mainMenu" style = "position:relative; left:1700px; top:-440px;">
    ...
    <div class="dailyreportanalysis" style = "position:relative; left:250px; top:-600px;">
      <p>The most people exited(<?php echo $max?>) was at <?php echo $maxtime  ?>
      <br>
      <?php $result = mysqli_query($conn,"SELECT MIN(NULLIF(PeopleExited,0)) AS min_people FROM DailyReportDataExited");
            $row = mysqli_fetch_array($result);
            $minpeople = $row['min_people'];
            $result2 = mysqli_query($conn,"SELECT TimeExited FROM DailyReportDataExited WHERE PeopleExited=$minpeople");
            $row2 = mysqli_fetch_array($result2);
            $mintime = $row2['TimeExited'];
      ?>
      <p>The least people exited(<?php echo $minpeople?>) was at <?php echo $mintime ?>
    </div>
